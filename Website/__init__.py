...
@app.route(entry_point+'/guidelines')
...
def upload():
	...
	if request.method == 'POST':
		...
		###### Catch Exception So that users are redirected to error page if thir submission is not accepted #######
		###### This is point at which users usually run into an error of some sort
		...
		#### Create separate catch exception for a duplicate submission name, so that a user knows that the problem is not with the file itself ############3
		...
		try: 
			...
			return render_template('duplicate_submission.html')
